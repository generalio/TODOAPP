...
class CustomChildLinearLayout(context: Context, attrs: AttributeSet?) : LinearLayout(context, attrs) {
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        when(event.action) {
            MotionEvent.ACTION_MOVE -> {
                val transitionX = (event.x - initialX).coerceIn(-deleteLayout.width.toFloat(), 0F)
                textLayout.translationX = transitionX
                deleteLayout.translationX = transitionX
            }
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                if(-deleteLayout.translationX > deleteLayout.width / 2) {
                    textLayout.animate().translationX(-deleteLayout.width.toFloat())
                    deleteLayout.animate().translationX(-deleteLayout.width.toFloat())
                } else {
